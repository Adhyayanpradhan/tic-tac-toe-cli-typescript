...
// console.log(args[4])
let socketio = require("socket.io-client")(args[3]);
let game : {board?: any, id?: number, player?:any} = {}
...
socketio.on('connect', () => {
  ...
  console.log(`Connected to the server. What would you like to do ${args[args.length - 1]}? `)
  inquirer.prompt(questions).then(ans => {
    // console.log(ans)
    if(ans["modes"] === "Create a new game") {
      socketio.emit("create")
    } else if (ans["modes"] === "Join a game") {
      inquirer.prompt(joinQuestion).then(ansJoin => {
        
        if(ansJoin["join"] !== null) {

          console.log(`Thanks for joining ${args[args.length - 1]}. Welcome onboard.`)
...
socketio.on('created', function (data) {
  // Show the GameID
  console.log(`Created a new game with id: ${data.id}. Send this id to Player-2 ${args[args.length - 1]}`)
});
...
socketio.on("roomsAvail",(val) => {
  ...
  inquirer.prompt(whichRooms).then(room => {
    console.log(`You entered room number - ${room["modes"]}`)
...
function keyToMove() {
  ...
  process.stdin.on('keypress', function(s, key) {
    ...
    if(game.player.turn === true) {
      ...
      if (game.board[s] === "") {
        let data = { id: game.id, cell: s, player: game.player };
